...
void CountryData::parse_all_files()
{
	...
	for (int i = 0; i < country_color_filenames.size(); i++)
	{
		// filenames are like this: Aachen.txt
		Dictionary country_colors;
		country_colors["Name"] = country_color_filenames[i].split(".")[0];
		String filename = country_color_filenames[i].get_file();
		String full_path = country_colors_folder_path.path_join(country_color_filenames[i]);
	...
	for (int i = 0; i < country_filenames.size(); i++)
	{
		Dictionary country_codes;
		// this is how the file names are AAC - Aachen.txt
		country_codes["Id"] = country_filenames[i].substr(0, 3); // gets AAC identifier
		country_codes["Name"] = country_filenames[i].substr(6).split(".")[0]; // get country name Aachen

		country_data.push_back(country_codes);
	}

	for (int i = 0; i < province_filenames.size(); i++)
	{
		//1-Uppland.txt
		Dictionary province_codes;
		String filename = province_filenames[i].get_file();
		...
		String full_path = provinces_folder_path.path_join(province_filenames[i]);
...
PackedStringArray CountryData::get_country_provinces(uint32_t country_index)
{
	...
	for (int j = 0; j < province_data.size(); j++)
	{
		Dictionary province_dict = province_data[j];
...
Color CountryData::get_country_color(const String &country_name)
{
	...
	{
		Dictionary color_entry = country_color_data[i];
